...
class AnimalSerializer(serializers.ModelSerializer[Animal]):
    colores = serializers.PrimaryKeyRelatedField(
        queryset=Color.objects.all(), many=True
    )
    raza: serializers.CharField = serializers.CharField(
        source="id_raza.nombre", read_only=True, required=False
    )
    tamaño: serializers.CharField = serializers.CharField(
        source="id_tamaño.nombre", read_only=True
    )
...
class PersonaSerializer(serializers.ModelSerializer[Persona]):
    felinos: AnimalSerializer = AnimalSerializer(
        source="felinos_cache", many=True, read_only=True
    )
    caninos: AnimalSerializer = AnimalSerializer(
        source="caninos_cache", many=True, read_only=True
    )
    domicilio_actual: DomicilioSerializer = DomicilioSerializer(
...
class AtencionSerializer(serializers.ModelSerializer[Atencion]):
    efector_nombre: serializers.CharField = serializers.CharField(
        source="id_efector.nombre", read_only=True
    )
    personal_nombre: serializers.CharField = serializers.CharField(
        source="personal_full_name", read_only=True
    )
    animal: AnimalSerializer = AnimalSerializer(
        source="id_animal", read_only=True, required=False
    )
    insumos: InsumoSerializer = InsumoSerializer(many=True, read_only=True)
    estado_egreso: serializers.CharField = serializers.CharField(
        source="id_estado_egreso.nombre", read_only=True, required=False
    )
...
class PersonalSerializer(serializers.ModelSerializer[Personal]):
    persona: PersonaSerializer = PersonaSerializer(
        source="id_persona", read_only=True, required=False
    )
