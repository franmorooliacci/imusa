...
const AnimalForm = ({
    ...
    const mapToFormValues = (animal: Animal): AnimalFormValues => {
        return {
            ...
            fecha_nacimiento: animal.fecha_nacimiento
                ? dayjs(animal.fecha_nacimiento).startOf('month').toDate()
                : undefined,
            ...
            es_peligroso: animal.es_peligroso,
    ...
    const idRaza = useWatch({ control, name: 'id_raza' });

    const selectedRaza = useMemo(() => {
        if (!idRaza) return null;
        return razas.find((r) => r.id === Number(idRaza)) ?? null;
    }, [idRaza, razas]);

    const razaEsPeligrosa = selectedRaza?.es_peligrosa === 1;

    useEffect(() => {
        if (razaEsPeligrosa) {
            setValue('es_peligroso', 1, {
                shouldValidate: true,
                shouldDirty: true,
            });
        }
    }, [razaEsPeligrosa, setValue]);
    ...
    const onSubmit = async (data: Record<string, any>): Promise<void> => {
        ...
        try {
            if (mode === 'add') {
                console.log(formatAnimal(data));
                const response: Animal = await addAnimal(formatAnimal(data));

                setAlertSeverity('success');
                setAlertMsg(
                    `${response.id_especie === 1 ? 'Canino' : 'Felino'} agregado con éxito!`,
                );
                setAlertOpen(true);

                setTimeout(() => {
                    onSuccess(
                        `/${response.id_especie === 1 ? 'canino' : 'felino'}/${response.id}`,
                    );
                }, 3000);
            } else {
                ...
                setAlertSeverity('success');
                setAlertMsg(
                    `${response.id_especie === 1 ? 'Canino' : 'Felino'} modificado con éxito!`,
                );
                setAlertOpen(true);
                setTimeout(() => {
                    onSuccess(response);
            ...
            if (mode === 'add') {
                setAlertSeverity('error');
                setAlertMsg(
                    `No se pudo agregar el ${data.id_especie === 1 ? 'canino' : 'felino'}. Por favor, inténtalo de nuevo más tarde.`,
                );
                setAlertOpen(true);
                setSubmitting(false);
            } else {
                setAlertSeverity('error');
                setAlertMsg(
                    `No se pudo modificar el ${data.id_especie === 1 ? 'canino' : 'felino'}. Por favor, inténtalo de nuevo más tarde.`,
                );
                setAlertOpen(true);
                setSubmitting(false);
            }
                ...
                >
                    <Divider>
                        ...
                        >
                            <FontAwesomeIcon
                                icon={
                                    initialData.id_especie === 1 ? faDog : faCat
                                }
                                size="2x"
                            />
                            <Typography variant="h5">
                                {mode === 'add' ? 'Agregar' : 'Editar'}{' '}
                                {initialData.id_especie === 1
                                    ? 'Canino'
                                    : 'Felino'}
                            </Typography>
                        </Box>
                    ...
                    <Grid2 container spacing={2} sx={{ width: '100%' }}>
                        ...
                        <Grid2 size={{ xs: 12, sm: 6, md: 6, lg: 4 }}>
                            <FormControl fullWidth error={!!errors.colores}>
                                <InputLabel id="pelaje">
                                    Pelaje (color)
                                </InputLabel>
                                <Controller
                                    name="colores"
                        ...
                        <Grid2 size={{ xs: 12, sm: 6, md: 6, lg: 4 }}>
                            {razaEsPeligrosa ? (
                                <TextField
                                    ...
                                    helperText="Definido por la raza"
                                    fullWidth
                                />
                            ) : (
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <TextField
                                            ...
                                            select
                                            label="Peligroso"
                                            error={!!errors.es_peligroso}
                                            helperText={
                                                errors.es_peligroso?.message
                                            }
                                            fullWidth
                                        >
                                            <MenuItem value={0}>No</MenuItem>
