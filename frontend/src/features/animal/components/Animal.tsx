...
const Animal = () => {
    ...
    const visibleAtenciones: Atencion[] = atenciones.filter(
        (a) => a.finalizada === 1,
    );
    const sortedAtenciones: Atencion[] =
        sortAtencionesAsc(visibleAtenciones).reverse();

    useEffect(() => {
        const fetchData = async () => {
            try {
                const [animalList, atencionesList] = await Promise.all([
                    getAnimalById(Number(animalId)),
                    getAtenciones({ id_animal: animalId }),
                ]);
    ...
    const handleAddAtencion = (): void => {
        if (animal.fallecido === 1) {
            ...
            setAlertMsg(
                'No se le puede agregar una atención a un animal fallecido.',
            );
            setAlertOpen(true);
            return;
        }
    ...
    const handleDownload = useCallback(
        async (id_atencion: number): Promise<void> => {
            try {
                ...
                setAlertSeverity('error');
                setAlertMsg(
                    'No se pudo descargar el informe. Por favor, inténtalo de nuevo más tarde.',
                );
                setAlertOpen(true);
            }
        },
    ...
    const handleSendInformeEmail = useCallback(
        async (id_atencion: number): Promise<void> => {
            try {
                const response = await getResponsableById(
                    Number(animal.id_responsable),
                );
                ...
                if (!email) {
                    setAlertSeverity('warning');
                    setAlertMsg(
                        `La persona no tiene asociada una dirección de correo.`,
                    );
                    setAlertOpen(true);
                    return;
                }
                ...
                setAlertSeverity('error');
                setAlertMsg(
                    'No se pudo enviar el informe. Por favor, inténtalo de nuevo más tarde.',
                );
                setAlertOpen(true);
            }
        },
                    ...
                    >
                        ...
                        <Tooltip title="Enviar por correo" arrow>
                            <IconButton
                                color="success"
                                onClick={() =>
                                    handleSendInformeEmail(atencion.id)
                                }
                                sx={{
                                    '&:hover': {
                                        backgroundColor: 'rgba(0, 0, 0, 0.08)',
        ...
        <Box>
            {editMode ? (
                <AnimalForm
                    mode={'edit'}
                    initialData={animal}
                    onSuccess={(updatedAnimal: AnimalType) => {
                ...
                <Grid2 container spacing={2} sx={{ width: '100%' }}>
                    <Grid2 size={{ xs: 12, sm: 12, md: 12, lg: 12 }}>
                        <BackHeader
                            navigateTo={`/responsable/${animal.id_responsable}`}
                        />
                    </Grid2>
                    <Grid2
                        size={{ xs: 12, sm: 12, md: 4, lg: 4 }}
                    ...
                    >
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    if (animal.fallecido === 1) {
                                        setAlertSeverity('warning');
                                        setAlertMsg(
                                            'No se puede editar la información de un animal fallecido.',
                                        );
                                        setAlertOpen(true);
                                        return;
                                    } else {
                        ...
                        <Stack spacing={0.5}>
                            ...
                            <Typography variant="body1">
                                ...
                                {animal.id_especie === 1 ? 'Canino' : 'Felino'}
                            </Typography>
                            ...
                            <Typography variant="body1">
                                ...
                                {animal.esterilizado === 1 ? 'Si' : 'No'}
                            </Typography>

                            <Typography variant="body1">
                                <strong>Adoptado en IMuSA:</strong>{' '}
                                {animal.adoptado_imusa === 1 ? 'Si' : 'No'}
                            </Typography>

                            <Typography variant="body1">
                                <strong>Peligroso:</strong>{' '}
                                {animal.es_peligroso === 1 ? 'Si' : 'No'}
                            </Typography>
                    ...
                    >
                        ...
                        >
                            <Box sx={{ display: 'flex', gap: 1 }}>
                                ...
                                <Typography variant="h5">Atenciones</Typography>
                            </Box>
